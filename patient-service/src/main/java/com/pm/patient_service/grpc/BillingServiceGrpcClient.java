...
public class BillingServiceGrpcClient {
    ...
    public BillingServiceGrpcClient(
            @Value("${BILLING_SERVICE_ADDRESS:localhost}") String serverAddresss,
            @Value("${BILLING_SERVICE_GRPC_PORT:9001}") int serverPort
    ){
        log.info("connecting to Billing Service GRPC service at {}:{}", serverAddresss, serverPort);

        ManagedChannel channel = ManagedChannelBuilder.forAddress(serverAddresss,serverPort)
                .usePlaintext().build();
